...
Iara Denise Endruweit Battisti, Felipe Micail da Silva Smolski \emph{(Organizadores)}
...
Dispondo de um banco de dados em uma planilha eletrônica (LibreOffice Calc ou Excel), neste caso será utilizado o arquivo {[}árvores{]} (\url{https://github.com/Smolski/livror/raw/master/arvores.xlsx}) como exemplo de banco de dados. Os dados derivam de uma pesquisa com espécies de árvores registrando as variáveis diâmetro altura do peito (DAP) e altura. Dados cedidos pela professora Tatiane Chassot.
...
Outras opções de arquivos podem ser carregados no RStudio, como por exemplo arquivos de texto (.txt ou .csv), arquivos derivados do excel (.xls ou .xlsx), arquivos de dados do SPSS (.sav), do \emph{software} SAS (.sas7bdat) e do STATA (.dta). A instalação de alguns pacotes é requerida, dependendo da origem da base de dados, como por exemplo o \texttt{readxl} (Wickham e Bryan, \protect\hyperlink{ref-readxl2018}{2018}) , \texttt{readr} (Wickham, Hester e Francois, \protect\hyperlink{ref-readr2018}{2018}) e \texttt{haven} (Wickham e Miller, \protect\hyperlink{ref-haven2018}{2018}), como os exemplos abaixo:
...
É possível carregar uma base de dados de determinado pacote instalado, como exemplo utilizado a partir do pacote \texttt{Amelia} (Honaker, King e Blackwell, \protect\hyperlink{ref-Amelia2011}{2011}):

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{data}\NormalTok{(africa, }\DataTypeTok{package=}\StringTok{"Amelia"}\NormalTok{)}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{class}\NormalTok{(iris}\OperatorTok{$}\NormalTok{Species)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Efetua-se a contagem dos dados qualitativos com a função table}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Adiciona-se a soma dos valores das linhas e colunas}
\KeywordTok{addmargins}\NormalTok{(}\KeywordTok{table}\NormalTok{(cdc}\OperatorTok{$}\NormalTok{genhlth,cdc}\OperatorTok{$}\NormalTok{gender))}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{starwars=}\KeywordTok{data.frame}\NormalTok{(starwars[}\DecValTok{1}\OperatorTok{:}\DecValTok{5}\NormalTok{])}
\KeywordTok{str}\NormalTok{(starwars)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{names}\NormalTok{(starwars)=}\KeywordTok{abbreviate}\NormalTok{(}\KeywordTok{names}\NormalTok{(starwars), }\DataTypeTok{minlength =} \DecValTok{3}\NormalTok{)}
\KeywordTok{names}\NormalTok{(starwars)}
\end{Highlighting}
\end{Shaded}
...
Caso o pesquisador deseje renomear todas as variáveis, a função \texttt{names()} pode ser utilizada como é mostrado abaixo, em um primeiro momento somente determinando o nome da primeira variável, e no segundo exemplo alterando todas as variáveis do objeto:

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{names}\NormalTok{(starwars)[}\DecValTok{1}\NormalTok{]=}\StringTok{"Nome"}
\KeywordTok{names}\NormalTok{(starwars)=}\KeywordTok{c}\NormalTok{(}\StringTok{"Nome"}\NormalTok{, }\StringTok{"Altura"}\NormalTok{, }\StringTok{"Peso"}\NormalTok{, }\StringTok{"Corcabelo"}\NormalTok{, }\StringTok{"Corpele"}\NormalTok{)}
\KeywordTok{names}\NormalTok{(starwars) }
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{starwars}\OperatorTok{$}\NormalTok{Corcabelo=}\KeywordTok{as.factor}\NormalTok{(starwars}\OperatorTok{$}\NormalTok{Corcabelo)}
\NormalTok{starwars}\OperatorTok{$}\NormalTok{Corpele=}\KeywordTok{as.factor}\NormalTok{(starwars}\OperatorTok{$}\NormalTok{Corpele)}
\KeywordTok{summary}\NormalTok{(starwars}\OperatorTok{$}\NormalTok{Corpele)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{head}\NormalTok{(}\KeywordTok{is.na}\NormalTok{(starwars))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{table}\NormalTok{(}\KeywordTok{is.na}\NormalTok{(starwars))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Substituindo NAs por média}
\NormalTok{starwars}\OperatorTok{$}\NormalTok{Altura[}\KeywordTok{is.na}\NormalTok{(starwars}\OperatorTok{$}\NormalTok{Altura)]=}\KeywordTok{mean}\NormalTok{(starwars}\OperatorTok{$}\NormalTok{Altura, }\DataTypeTok{na.rm=}\OtherTok{TRUE}\NormalTok{)}
\KeywordTok{summary}\NormalTok{(starwars}\OperatorTok{$}\NormalTok{Altura)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Substituindo NAs por zero}
\NormalTok{starwars}\OperatorTok{$}\NormalTok{Peso[}\KeywordTok{is.na}\NormalTok{(starwars}\OperatorTok{$}\NormalTok{Peso)]=}\DecValTok{0}
\KeywordTok{summary}\NormalTok{(starwars}\OperatorTok{$}\NormalTok{Peso)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{starwars}\OperatorTok{$}\NormalTok{Peso[starwars}\OperatorTok{$}\NormalTok{Peso}\OperatorTok{==}\DecValTok{0}\NormalTok{]=}\OtherTok{NA}
\KeywordTok{summary}\NormalTok{(starwars}\OperatorTok{$}\NormalTok{Peso)}
\end{Highlighting}
\end{Shaded}
...
Ainda, se houver no objeto linhas que tenham pelo menos uma informação faltante (``NA''), estas podem ser excluídas com o comando \texttt{na.omit()}. Note que com este procedimento o objeto \texttt{starwars} passou a ter a dimensão 54x5 ao invés de 87x5 como apresentado inicialmente com a apressentação de valores faltantes:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{starwars=}\KeywordTok{na.omit}\NormalTok{(starwars)}
\KeywordTok{summary}\NormalTok{(starwars)}
\end{Highlighting}
\end{Shaded}
...
\KeywordTok{head}\NormalTok{(informacoes}\FloatTok{.3}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\KeywordTok{head}\NormalTok{(informacoes)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{classificacao=}\KeywordTok{ifelse}\NormalTok{(informacoes}\OperatorTok{$}\NormalTok{Imc}\OperatorTok{<}\DecValTok{25}\NormalTok{, }\StringTok{"peso normal"}\NormalTok{,}\StringTok{"excesso de peso"}\NormalTok{)}
...
Nesta subseção será utilizado o pacote \texttt{tidyr} (Wickham e Henry, \protect\hyperlink{ref-tidyr2018}{2018}) para demonstrar algumas funções que contribuem para a manipulação das bases de dados, tão importante no processo de preperação das informações para posterior análise. Serão utilizadas para demonstração as bases de dados existentes no próprio pacote.
...
Fonte: \url{http://garrettgman.github.io/tidying/}.
...
Utilizando o pacote \texttt{stringr} (Wickham, \protect\hyperlink{ref-stringr2018}{2018}) com sua função \texttt{str\_detect()}, é possível efetuar a filtragem pelo nome/identificação total ou parcial de um valor contido dentro de uma variável do banco de dados. Utilizou-se o banco de dados ``table5'' visto no subcapítulo anterior pra filtrar as informações da variável ``country'' que contém ``Bra'':

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{library}\NormalTok{(stringr)}
\NormalTok{table5 }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{filter}\NormalTok{(}\KeywordTok{str_detect}\NormalTok{(country, }\StringTok{"Bra"}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{novo =}\StringTok{ }\NormalTok{mtcars }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{filter}\NormalTok{(am }\OperatorTok{==}\StringTok{ }\DecValTok{1}\NormalTok{) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{group_by}\NormalTok{(cyl) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{summarise}\NormalTok{(}\DataTypeTok{disp.drat=}\KeywordTok{mean}\NormalTok{(drat), }
            \DataTypeTok{hp.media=}\KeywordTok{mean}\NormalTok{(hp))}
...
\StringTok{  }\KeywordTok{select}\NormalTok{(}\KeywordTok{starts_with}\NormalTok{(}\StringTok{"d"}\NormalTok{)) }\OperatorTok{%>%}
...
\begin{Highlighting}[]
\NormalTok{mtcars }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{select}\NormalTok{(}\KeywordTok{contains}\NormalTok{(}\StringTok{"a"}\NormalTok{)) }\OperatorTok{%>%}
\StringTok{  }\NormalTok{head}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\NormalTok{mtcars }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{aggregate}\NormalTok{(. }\OperatorTok{~}\StringTok{ }\NormalTok{cyl, ., mean)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{abertura <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"03/02/69"}\NormalTok{) }\CommentTok{# Exemplo de criação de data inicial}
\NormalTok{fechamento <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"2000-20-01"}\NormalTok{) }\CommentTok{# Criação de data final}
\NormalTok{abertura <-}\StringTok{ }\KeywordTok{as.Date}\NormalTok{(abertura, }\DataTypeTok{format =} \StringTok{"%d/%m/%y"}\NormalTok{) }\CommentTok{# Formatação da data}
\NormalTok{fechamento <-}\StringTok{ }\KeywordTok{as.Date}\NormalTok{(fechamento, }\DataTypeTok{format =} \StringTok{"%Y-%d-%m"}\NormalTok{) }
\KeywordTok{class}\NormalTok{(abertura) }\CommentTok{# Verificando a classe do objeto}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Configurando a variável data}
\NormalTok{data}\OperatorTok{$}\NormalTok{data=}\KeywordTok{dmy}\NormalTok{(data}\OperatorTok{$}\NormalTok{data)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Criando uma nova variável do ano da venda}
\NormalTok{data}\OperatorTok{$}\NormalTok{ano=}\KeywordTok{year}\NormalTok{(data}\OperatorTok{$}\NormalTok{data)}
\NormalTok{data}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Criando uma nova variável do mês da venda}
\NormalTok{data}\OperatorTok{$}\NormalTok{mes=}\KeywordTok{month}\NormalTok{(data}\OperatorTok{$}\NormalTok{data)}
\NormalTok{data}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Criando nova variável do dia da venda}
\NormalTok{data}\OperatorTok{$}\NormalTok{dia=}\KeywordTok{day}\NormalTok{(data}\OperatorTok{$}\NormalTok{data)}
\NormalTok{data}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Criando nova variável do dia da semana da venda}
\NormalTok{data}\OperatorTok{$}\NormalTok{diasem=}\KeywordTok{wday}\NormalTok{(data}\OperatorTok{$}\NormalTok{data, }\DataTypeTok{label=}\OtherTok{TRUE}\NormalTok{)}
\NormalTok{data}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Valor das vendas por mês}
\KeywordTok{aggregate}\NormalTok{(data}\OperatorTok{$}\NormalTok{valor, }\KeywordTok{list}\NormalTok{(}\DataTypeTok{Var =}\NormalTok{ data}\OperatorTok{$}\NormalTok{mes), sum)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Quantidade de vendas por mês}
\KeywordTok{aggregate}\NormalTok{(data}\OperatorTok{$}\NormalTok{quant, }\KeywordTok{list}\NormalTok{(}\DataTypeTok{Var =}\NormalTok{ data}\OperatorTok{$}\NormalTok{mes), sum)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Utilizando o pacote dplyr}
\KeywordTok{library}\NormalTok{(dplyr)}

\NormalTok{data }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{group_by}\NormalTok{(diasem) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{summarise}\NormalTok{(}\DataTypeTok{total =} \KeywordTok{sum}\NormalTok{(valor))}
\end{Highlighting}
\end{Shaded}
...
{\centering \includegraphics[width=0.7\linewidth]{index_files/figure-latex/unnamed-chunk-111-1} 

}

\caption{Gráfico de colunas com a variável Sexo}\label{fig:unnamed-chunk-111}
\end{figure}
...
\caption{Gráfico de colunas com a variável Sexo (Horizontal)}\label{fig:unnamed-chunk-112}
\end{figure}
...
\caption{Gráfico de colunas com a variável Pessoas familia}\label{fig:unnamed-chunk-113}
\end{figure}
...
\caption{Gráfico de colunas com as variáveis Sexo e Divulgacao}\label{fig:unnamed-chunk-114}
\end{figure}
...
{\centering \includegraphics[width=0.7\linewidth]{index_files/figure-latex/unnamed-chunk-116-1} 

}

\caption{Gráfico de colunas com as variáveis Sexo e Divulgacao (2)}\label{fig:unnamed-chunk-116}
\end{figure}
...
\CommentTok{## Alguns exemplos de paletas de cores:}
...
\caption{Gráfico de pizza com a variável Sabor}\label{fig:unnamed-chunk-117}
\end{figure}
...
\caption{Histograma com a variável `Renda h`}\label{fig:unnamed-chunk-118}
\end{figure}
...
\caption{Histograma com a variável Renda h com breaks=5}\label{fig:unnamed-chunk-119}
\end{figure}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{hist}\NormalTok{(}\KeywordTok{as.numeric}\NormalTok{(}\StringTok{`}\DataTypeTok{Renda_h}\StringTok{`}\NormalTok{), }\DataTypeTok{breaks=}\DecValTok{5}\NormalTok{)}\OperatorTok{$}\NormalTok{counts}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{hist}\NormalTok{(}\KeywordTok{as.numeric}\NormalTok{(}\StringTok{`}\DataTypeTok{Renda_h}\StringTok{`}\NormalTok{), }\DataTypeTok{breaks=}\DecValTok{5}\NormalTok{)}\OperatorTok{$}\NormalTok{breaks}
\end{Highlighting}
\end{Shaded}

\begin{center}\includegraphics[width=0.7\linewidth]{index_files/figure-latex/unnamed-chunk-120-1} \end{center}
...
{\centering \includegraphics[width=0.7\linewidth]{index_files/figure-latex/unnamed-chunk-121-1} 

}

\caption{Boxplot com a variável Idade}\label{fig:unnamed-chunk-121}
\end{figure}
...
\caption{Boxplot com as variáveis Idade e Sabor}\label{fig:unnamed-chunk-122}
\end{figure}
...
Fonte: DIEESE (\protect\hyperlink{ref-Dieese2011}{2011}).
...
{\centering \includegraphics[width=0.7\linewidth]{index_files/figure-latex/unnamed-chunk-125-1} 

}

\caption{Gráfico de linhas sobre a fiscalização do trabalho na área rural Brasil 1998-2010}\label{fig:unnamed-chunk-125}
\end{figure}

Fonte: Elaborado pelo(s) autor(es) a partir de DIEESE (\protect\hyperlink{ref-Dieese2011}{2011}).
...
\NormalTok{inmet <-}\StringTok{ }\KeywordTok{read_delim}\NormalTok{(}\StringTok{"https://goo.gl/2p11WS"}\NormalTok{, }
...
{\centering \includegraphics[width=0.7\linewidth]{index_files/figure-latex/unnamed-chunk-127-1} 

}

\caption{Gráfico de linha sobre as temperaturas registradas em São Luiz Gonzaga - RS}\label{fig:unnamed-chunk-127}
\end{figure}

Fonte: Elaborado pelo(s) autor(es) a partir de INMET (\protect\hyperlink{ref-inmet}{2018}).
...
\caption{\label{tab:unnamed-chunk-160}Amostras dependentes}
\centering
...
\caption{\label{tab:unnamed-chunk-162}Amostras dependentes - caso 2}
\centering
...
\caption{\label{tab:unnamed-chunk-164}Comparação de dois tipos diferentes de tecidos}
\centering
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{cor.test}\NormalTok{(tempo, nota)}
\end{Highlighting}
\end{Shaded}
...
Ainda, com o pacote \texttt{PerformanceAnalytics} (Peterson e Carl, \protect\hyperlink{ref-PerformanceAnalytics}{2018}) é possível aprimorar a exibição dos resultados sobre o coeficiente de correlação com as variáveis observadas, que além do coeficientes de correlação, mostra a distribuição das variáveis e o gráfico de dispersão:

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{library}\NormalTok{(}\StringTok{"PerformanceAnalytics"}\NormalTok{)}
\KeywordTok{chart.Correlation}\NormalTok{(}\KeywordTok{cbind}\NormalTok{(tempo,nota), }\DataTypeTok{histogram=}\OtherTok{TRUE}\NormalTok{, }\DataTypeTok{pch=}\DecValTok{19}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{center}\includegraphics[width=0.7\linewidth]{index_files/figure-latex/unnamed-chunk-183-1} \end{center}
...
As expressões para os coeficientes, que minimizam a Soma de Quadrado dos Resíduos são obtidas pela derivada desta soma de quadrados em relação a \(b_0\) e em relação a \(b_1\) e podem ser descritas por (Hoffmann e Vieira, \protect\hyperlink{ref-hoffmann1998}{1998}):
...
\texttt{nome\_para\_regressao=lm(variável\_dependente\ \textasciitilde{}\ variável\_preditora)}

Por exemplo:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{regressaolinear=}\KeywordTok{lm}\NormalTok{(nota}\OperatorTok{~}\NormalTok{tempo)}
\NormalTok{regressaolinear}
\end{Highlighting}
\end{Shaded}
...
\textbf{FV}: Fonte de Variação;

\textbf{GL}: Grau de Liberdade;

\textbf{SQ}: Soma de Quadrados;

\textbf{QM}: Quadrados Médios;
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{anova}\NormalTok{(regressaolinear)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{summary}\NormalTok{(regressaolinear)}
\end{Highlighting}
\end{Shaded}
...
\KeywordTok{abline}\NormalTok{(regressaolinear)}
\end{Highlighting}
\end{Shaded}

\begin{figure}[H]

{\centering \includegraphics[width=0.7\linewidth]{index_files/figure-latex/unnamed-chunk-186-1} 

}

\caption{Reta de regressão ajustada da nota em relação ao tempo de estudo dos participantes da pesquisa}\label{fig:unnamed-chunk-186}
\end{figure}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{confint}\NormalTok{(regressaolinear)}
\end{Highlighting}
\end{Shaded}
...
\KeywordTok{predict}\NormalTok{(regressaolinear, x0, }\DataTypeTok{interval=}\StringTok{"prediction"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\texttt{plot(fitted(nome\_para\_regressao),\ residuals(nome\_para\_regressao),}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{plot}\NormalTok{(}\KeywordTok{fitted}\NormalTok{(regressaolinear), }\KeywordTok{residuals}\NormalTok{(regressaolinear),}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{plot}\NormalTok{(tempo, }\KeywordTok{residuals}\NormalTok{(regressaolinear), }
     \DataTypeTok{xlab =} \StringTok{"Tempo"}\NormalTok{,}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{qqnorm}\NormalTok{(}\KeywordTok{residuals}\NormalTok{(regressaolinear))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{hist}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ regressaolinear}\OperatorTok{$}\NormalTok{residuals,}
...
\KeywordTok{lines}\NormalTok{(}\KeywordTok{density}\NormalTok{(regressaolinear}\OperatorTok{$}\NormalTok{residuals))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{shapiro.test}\NormalTok{(}\KeywordTok{residuals}\NormalTok{(regressaolinear))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{rstudent}\NormalTok{(regressaolinear)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{rstandard}\NormalTok{(regressaolinear)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{plot}\NormalTok{(}\KeywordTok{rstandard}\NormalTok{(regressaolinear))}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{plot}\NormalTok{(}\KeywordTok{rstudent}\NormalTok{(regressaolinear)) }
\KeywordTok{abline}\NormalTok{(}\DataTypeTok{h=}\DecValTok{2}\NormalTok{,}\DataTypeTok{col=}\StringTok{"red"}\NormalTok{)}
\KeywordTok{abline}\NormalTok{(}\DataTypeTok{h=}\OperatorTok{-}\DecValTok{2}\NormalTok{,}\DataTypeTok{col=}\StringTok{"red"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{dffits}\NormalTok{(regressaolinear)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{plot}\NormalTok{(}\KeywordTok{dffits}\NormalTok{(regressaolinear))}
...
pander\footnote{Daróczi e Tsegelskyi (\protect\hyperlink{ref-pander2018}{2018}).} & ok & ok & ok\tabularnewline
...
\textbf{Djaina Sibiani Rieger}: Acadêmica do curso de Matemática da Universidade Federal da Fronteira Sul (UFFS) Campus Chapecó, foi aluna bolsista de extensão, integrante e conteudista dos cursos ofertados no Campus Cerro Largo sobre o \emph{software} livre R.
...
DAHL, D. B. \emph{et al.} \textbf{xtable: Export Tables to LaTeX or HTML}, 2018. Disponível em: \textless{}\url{https://CRAN.R-project.org/package=xtable}\textgreater{}

\leavevmode\hypertarget{ref-pander2018}{}%
DARÓCZI, G.; TSEGELSKYI, R. \textbf{pander: An R 'Pandoc' Writer}, 2018. Disponível em: \textless{}\url{https://CRAN.R-project.org/package=pander}\textgreater{}

\leavevmode\hypertarget{ref-Dieese2011}{}%
DIEESE, D. \textbf{Estatísticas do meio rural 2010-2011}.. 6. ed. São Paulo: DIEESE, 2011.
...
HLAVAC, M. \textbf{stargazer: Well-Formatted Regression and Summary Statistics Tables}.. Bratislava, Slovakia: Central European Labour Studies Institute (CELSI), 2018.
...
INMET. \textbf{Insituto Nacional de Meteorologia}, 2018. Disponível em: \textless{}\url{http://www.inmet.gov.br/portal/}\textgreater{}. Acesso em: 1 dez. 2018
...
PETERSON, B. G.; CARL, P. \textbf{PerformanceAnalytics: Econometric Tools for Performance and Risk Analysis}, 2018. Disponível em: \textless{}\url{https://CRAN.R-project.org/package=PerformanceAnalytics}\textgreater{}
...
WICKHAM, H. \textbf{stringr: Simple, Consistent Wrappers for Common String Operations}, 2018. Disponível em: \textless{}\url{https://CRAN.R-project.org/package=stringr}\textgreater{}

\leavevmode\hypertarget{ref-readxl2018}{}%
WICKHAM, H.; BRYAN, J. \textbf{readxl: Read Excel Files}, 2018. Disponível em: \textless{}\url{https://CRAN.R-project.org/package=readxl}\textgreater{}

\leavevmode\hypertarget{ref-dplyr2019}{}%
WICKHAM, H. \emph{et al.} \textbf{dplyr: A Grammar of Data Manipulation}, 2019. Disponível em: \textless{}\url{https://CRAN.R-project.org/package=dplyr}\textgreater{}

\leavevmode\hypertarget{ref-tidyr2018}{}%
WICKHAM, H.; HENRY, L. \textbf{tidyr: Easily Tidy Data with 'spread()' and 'gather()' Functions}, 2018. Disponível em: \textless{}\url{https://CRAN.R-project.org/package=tidyr}\textgreater{}

\leavevmode\hypertarget{ref-readr2018}{}%
WICKHAM, H.; HESTER, J.; FRANCOIS, R. \textbf{readr: Read Rectangular Text Data}, 2018. Disponível em: \textless{}\url{https://CRAN.R-project.org/package=readr}\textgreater{}

\leavevmode\hypertarget{ref-haven2018}{}%
WICKHAM, H.; MILLER, E. \textbf{haven: Import and Export 'SPSS', 'Stata' and 'SAS' Files}, 2018. Disponível em: \textless{}\url{https://CRAN.R-project.org/package=haven}\textgreater{}

\leavevmode\hypertarget{ref-knitr2018}{}%
XIE, Y. \textbf{knitr: A General-Purpose Package for Dynamic Report Generation in R}, 2018. Disponível em: \textless{}\url{https://CRAN.R-project.org/package=knitr}\textgreater{}
