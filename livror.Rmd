...
```{r paineis, echo=FALSE, fig.cap='Painéis do Rstudio',fig.subcap = c("Fonte: Elaborado pelo(s) autor(es).")}
knitr::include_graphics("paineis.png")
```
...
```{r pacotes1, echo=FALSE, fig.cap='Instalação de pacotes',fig.subcap = c("Fonte: Elaborado pelo(s) autor(es).")}
knitr::include_graphics("pacotes,1.png")
```

```{r pacotes2, echo=FALSE, fig.cap='Caixa de informação de pacote a ser instalado',fig.subcap = c("Fonte: Elaborado pelo(s) autor(es).")}
knitr::include_graphics("pacotes,2.png")
```
...
```{r r3, echo=FALSE, fig.cap='Aba *Import Dataset*',fig.subcap = c("Fonte: Elaborado pelo(s) autor(es).")}
knitr::include_graphics("r3.png")
```

Na caixa correspondente a File/Url se insere o endereço virtual ou o local onde se encontra o arquivo. Ao importar os dados, carrega-se um objeto criado com as informações contidas no arquivo. No nosso exeplo, carregamos a planilha arvores (arquivo .xls) como mostra a Figura \@ref(fig:r4), derivado do caminho "Import Dataset $>$ From Excel" do Environment.

```{r r4, echo=FALSE, fig.cap='Caixa de informações do Import Data',fig.subcap = c("Fonte: Elaborado pelo(s) autor(es).")}
knitr::include_graphics("r4.png")
```
...
```{r r4csv, echo=FALSE, fig.cap='Opções da importação de arquivos .csv',fig.subcap = c("Fonte: Elaborado pelo(s) autor(es).")}
knitr::include_graphics("r4csv.png")
```
...
```{r r6, echo=FALSE, fig.cap='Atalho para abrir e salvar arquivo de dados',fig.subcap = c("Fonte: Elaborado pelo(s) autor(es).")}
knitr::include_graphics("r6.png")
```
...
**Exemplo 1**: Uma pesquisa sobre "a exposição a agrotóxicos entre trabalhadores rurais no município de Cerro Largo/RS" foi desenvolvida por Letiane Peccin Ristow, no ano de 2017 (dissertação e mestrado no Programa de Pós-Graduação em Desenvolvimento e Políticas Públicas da UFFS, Campus Cerro Largo. Na Tabela \@ref(tab:qui2) são apresentados os resultados do "tamanho da propriedade" e "armazenamento seguro do EPI". Para verificar a existência de associação significativa entre essas duas variáveis utilizamos o teste de qui-quadrado, dado que são duas variáveis qualitativas: variável 1 - tamanho da propriedade (até 25ha; 26ha ou mais) e variável 2 – armazenamento seguro (sim; não).
...
Realizar o teste de associação para os dados da Tabela \@ref(tab:qui2), para isso, digitar os dados da tabela cruzada (tabela de contingência) no formato de uma matriz, valor ij, considerando i=linha e j=coluna, em sequência por coluna (por exemplo, digita-se todos os valores da primeira coluna, depois digita-se todos os valores da segunda coluna e assim sucessivamente).
...
O segundo comando `quiquadrado1`, mostra a matriz elaborada, que neste caso representa uma tabela cruzada de duas linhas e duas colunas, conforme a Tabela \@ref(tab:qui2).
...
Caso pelo menos uma casela tenha frequência esperada menor que 5 como por exemplo na Tabela \@ref(tab:qui3), utilizamos o teste exato de Fisher.
...
**Exemplo 2**: Uma pesquisa foi realizada para verificar o efeito de um medicamento para perda de peso. O estudo foi realizado com 45 cobaias com características semelhantes. Na Tabela \@ref(tab:qui5) são apresentadas a situação do peso antes e após a intervenção (utilização do medicamento). 
...
```{r qui5, echo=FALSE, fig.subcap="Situação do peso de cobaias do estudo antes e após a intervenção"}
require(kableExtra)
